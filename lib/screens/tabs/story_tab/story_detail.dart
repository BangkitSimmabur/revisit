...
class StoryDetail extends StatefulWidget {
  final Story storyExplore;

  StoryDetail({
    @required this.storyExplore,
  });
...
class _StoryDetailState extends State<StoryDetail> {
  ...
  Future initDataState() async {
    await storyService.getStoryById(widget.storyExplore.id);

    var like = LikeUtils().checkIfLiked(
      storyService.currentStoryV2.likes,
      authService.currentUser.id,
    );

    setState(() {
      isLiked = like;
      likeCount = storyService.currentStoryV2.likes.length;
      isLoading = false;
      storyComments = storyService.currentStoryV2.comments;
    });
    ...
    return Column(
      children: [
        ...
        RevisitImgPlaceHolder(
          imgSrc: storyService.currentStoryV2.picture,
          fit: BoxFit.cover,
        ),
        _locationElement,
        Container(
  ...
  Widget get _mapComponent {
    if (storyService.currentStoryV2.location == null) {
      return Container();
    }

    return DetailMapElement(storyService.currentStoryV2.location);
  }
  ...
  Widget get _locationElement {
    return Padding(
      ...
      child: GestureDetector(
        onTap: () {
          _onMapPressed(storyService.currentStoryV2.location);
        },
        child: Row(
          ...
          children: [
            ...
            Text(
              storyService.currentStoryV2.location.address,
              style: TextStyle(
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  ClipOval(
                    child: storyService.currentStoryV2.user.profilePicture
                                .thumbnailImage !=
                            null
                        ? Image.network(
                            storyService.currentStoryV2.user.profilePicture
                                .thumbnailImage,
                            fit: BoxFit.cover,
                            width: 40,
                            height: 40,
                  ...
                  Text(
                    storyService.currentStoryV2.user.username,
                    style: TextStyle(
                      color: Colors.black,
              ...
              Text(
                storyService.currentStoryV2.text,
                style: TextStyle(
                  fontSize: Constant.MINIMUM_FONT_SIZE,
                  color: Constant.GRAY_TEXT,
                ),
              ...
              Text(
                storyService.currentStoryV2.createdAt,
                style: TextStyle(
                  fontSize: Constant.MINIMUM_FONT_SIZE_XS,
                  color: Constant.GRAY_TEXT,
                ),
  ...
  Future<void> _onLikePressed() async {
    MainPlatform.showLoadingAlert(
        context, !isLiked ? "Menyukai cerita" : "Tidak menyukai cerita");

    var serverLog =
        await storyService.likeStory(storyService.currentStoryV2.id);
  ...
  Future<void> _onCreateComment() async {
    ...
    var serverLog = await storyService.createComment(
      storyService.currentStoryV2.id,
  ...
  Future<void> _onReport() async {
    ...
    await showDialog(
        context: context,
        builder: (_) => StoryReportDialog());
  }
}

class StoryReportDialog extends StatefulWidget {

  @override
  _StoryReportDialogState createState() => _StoryReportDialogState();
}

class _StoryReportDialogState extends State<StoryReportDialog> {
  ...
  Future<void> _onReportStory() async {
    ...
    if (_reportController.text != "" && _titleController.text != "") {
      MainPlatform.showLoadingAlert(context, 'Melaporkan Cerita');

      var serverLog = await _storyService.reportStory(
        _storyService.currentStoryV2.id,
      ...
      if (serverLog.success) {
        MainPlatform.backTransitionPage(context);
        MainPlatform.backTransitionPage(context);
        MainPlatform.showSuccessSnackbar(context, 'Cerita dilaporkan');
        return;
      }
